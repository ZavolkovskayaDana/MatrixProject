...
void AppManager::createNewLine() {
    ...
    figures.push_back(new Line(speed, length, epilepsy)); //функци€ возвращает указатель и кладет во внутренний массив
...
void AppManager::drawFrame() {
    ...
    for (auto it = figures.begin(); it != figures.end(); ) 
    {
        if ((*it)->isFinished()) { //если фигура закончила жиз цикл
            delete* it;  //удал€ем ее с помощью нашего вир деструктора(вызовет правильный деструктор Line или  Explosion)
